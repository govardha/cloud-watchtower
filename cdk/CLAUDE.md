...
- `account=sandbox|development|production` → one `WorkloadWriterStack`: a
  ...
  EC2, ECS, Lambda) to write logs into its own account prefix.
...
| `configs/models.py` | Dataclasses (`LogArchiveConfig`, `WriterConfig`, `ReaderConfig`, `InfrastructureSpec`), hydrated by dacite. |
| `stacks/log_archive_stack.py` | Logarchive side, per region. |
| `stacks/workload_writer_stack.py` | Workload side, per account. |
...
Set `ACCOUNT=logarchive|sandbox|development|production` (default `logarchive`).
The matching AWS SSO profile is `admin-<ACCOUNT>`.
...
- **IAM least privilege:** explicit actions (no wildcards), explicit resource
  ...
  no delete anywhere. Trust-policy `Sid`s must be **alphanumeric**.
...
- Commit secrets, `.env`, or credentials (`.env` is gitignored; `.env.example`
  is tracked). Account IDs are fine (public within the org).
